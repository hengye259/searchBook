package org.hengsir.searchBook.utils;
...
import org.hengsir.searchBook.comparator.MyComparator;
...
public class BookSearchUtil {
	...
	public static void main(String[] args) {
		List<Book> list = searchKFZBooks("head first java");
		for (Book book : list) {
	...
	public static List<Book> searchKFZBooks(String bookTypeOrName) {
		...
		String url = "http://search.kongfz.com/product_result/?status=0&key="+u;
		Document d = getDocByUrl(url);
		//System.out.println(d.html());
		// 获取所以书本信息
		Elements boxs = d.select(".item.clearfix");
		System.out.println(boxs.html());
		try {
			for (Element element : boxs) {
				Book book = new Book();
				book.setImgSrc(element.select("a.img-box img").attr("src"));
				System.out.println(element.select("a.img-box img").attr("src"));
				book.setBookName(element.select("div.title a").text());
				book.setInner(element.select("div.title a").attr("href"));
				book.setAuthor(element.select("div.zl-isbn-info span").text());
				book.setPrice(element.select("div.price span.bold").text());
				book.setCompare(book.getPrice().substring(0, book.getPrice().indexOf(".")));
				System.out.println(book.getPrice().substring(0, book.getPrice().indexOf(".")));
				KFZBooks.add(book);
			}
		} catch (Exception e) {
